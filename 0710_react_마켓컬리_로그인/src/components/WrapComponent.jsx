...
export default function WrapComponent(){
    ...
    React.useEffect(()=>{
       ...
       if(localStorage.getItem(signInKey)!==null){
        ...
        if(new Date() > result.expires){
            alert("만료일이 지났습니다. 로그아웃 되었습니다.");
            setSignin({
                ...state,
                user_id:'',
                expires:''
            })
            localStorage.removeItem(signInKey); //로그인정보 모두 삭제
        }
        else{
            setSignin({
                ...state,
   ...
   const postCodeOpen=()=>{
    ...
    return (
        <div id='wrap'>
            <GlobalContext.Provider value={{CARTPRODUCTKEY,VIEWPRODUCTKEY,TOPMODALKEY,ADDRESSKEY,isAdmin,mainModal,mainModalshowOut,mainModalAyearshowOut,addressAuto,postCodeOpen,postCodeClose,login,setLogin,signin,setSignin}}>
