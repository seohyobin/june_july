...
export default function WrapComponent(){
    ...
    //새로고침시 자동 실행
    const addressAuto=()=>{
        if(sessionStorage.getItem('JANEADDRESS')!==null){
            const 주소1=JSON.parse(sessionStorage.getItem('JANEADDRESS')).주소1;
            const 주소2=JSON.parse(sessionStorage.getItem('JANEADDRESS')).주소2;
            setAddr({
                ...addr,
                주소1:주소1,
                주소2:주소2,
        ...
        <div id='wrap'>
            {
                topModal.isTopModal&&<TopModalComponent  topModalClose={topModalClose}/>
                    ...
                    <BrowserRouter basename={process.env.PUBLIC_URL}>
                        <Routes>
                            <Route path='/' element={<HeaderComponent cartCountNunber={cartCountNunber} setIsIntroFn={setIsIntroFn} MapText={state.MapText} isMap={state.isMap} cartCount={cartCount} addr={addr} openPopupDaumPostApi={openPopupDaumPostApi}/>}>
                                ...
                                <Route path='/signup' element={<SignUpComponent timer={state} timerCounterfn={timerCounterfn}  mapAddressFn={mapAddressFn} confirmModalOpen={confirmModalOpen} addr={addr} openPopupDaumPostApi={openPopupDaumPostApi}/>}/>
                                <Route path='/product' element={<ProductComponent key={key} cartCountNunber={cartCountNunber}/>}/>
                                <Route path='/cart' element={<CartComponent key={key}  confirmModalOpen={confirmModalOpen} addr={addr} openPopupDaumPostApi={openPopupDaumPostApi}/>}/>
                            </Route>
                        </Routes>
                    </BrowserRouter>
