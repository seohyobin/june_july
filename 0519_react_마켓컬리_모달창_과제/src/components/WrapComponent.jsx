...
import IntroModal from './wrap/IntroModal';
...
export default function WrapComponent() {
    ...
    const SetViewProduct=(value)=>{
        ...
        localStorage.setItem(product.key,JSON.stringify(value)); //객체라서Json 넣어!!!
    }
    ...
    /////////인트로 모달 쿠키
    const [inCookie,setInCookie] =React.useState({
        key:'HB_KURLY_INTROMODAL',
        isIntroCookie:true
    })
    //2-1 인트로 쿠키닫기 함수
    const IntroCookieClose=(value,expires)=>{
        setInCookie({
            ...inCookie,
           
        })
        setIntroCookieMethod(value,expires);
        
    }
    //2-2 쿠키 설정(저장)하기 : 탑모달은 닫기 클릭하면 쿠키 설정 저장!!
    const setIntroCookieMethod=(value,expires)=>{
        let today =new Date();
        today.setDate(today.getDate()+expires);
        document.cookie= `${inCookie.key}=${value}; path=/; expires=${today.toUTCString()};`;
    } 

    const getIntroCookieMethod=()=>{
        ...
        cookie.map((item)=>{
            if(item.key.includes(inCookie.key) && item.value.includes('yes')){
                setInCookie({
                    ...inCookie,
                    isIntroCookie:false
                })
    ...
    React.useEffect(()=>{
        getIntroCookieMethod();
    },[inCookie.isIntroCookie])
        ...
        <div id='wrap'>
            {
                ...
                <FooterComponent/>
                {
                   intro.isIntroModal && <IntroModal introModalClose={introModalClose} IntroCookieClose={IntroCookieClose}/>
                }       
                <GoTopComponent/>
                <QuickMenuComponent/>
